...
description: Vérifiez les paramètres de configuration dans le [!UICONTROL PayPal Payments Standard] de la section [!UICONTROL Sales] &gt; [!UICONTROL Payment Methods] de l’administrateur Commerce.
...
>À compter du 14 septembre 2019, les banques européennes pourraient refuser les paiements qui ne satisfont pas [PSD2](../../getting-started/compliance-payment-services-directive.md) conditions requises. Aucune action n’est nécessaire pour [!DNL PayPal Payments Standard] pour se conformer à PSD2, car toutes les exigences sont gérées par PayPal.
...
| [!UICONTROL Billing Agreement Signup] | Site Web | Détermine si les clients peuvent entrer dans une [contrat de facturation](../../stores-purchase/paypal-billing-agreements.md) avec votre magasin. Options : <br/>**`Auto`**- Le client peut s’inscrire à un contrat de facturation lors du passage en caisse express.<br/>**`Ask Customer`** - Les clients sont invités à s’inscrire à un contrat de facturation. <br/>**`Never`**- Les clients ne peuvent pas s’inscrire à un contrat de facturation. |
...
![Paramètres de contrat de facturation](./assets/payment-methods-paypal-express-checkout-billing-agreement-settings.png)<!-- zoom -->
...
| [!UICONTROL Payment Action] | Site Web | Détermine la manière dont PayPal gère la transaction : Options : <br/>**`Authorization`**- Valide l’achat, mais met un frein aux fonds. Le montant n’est pas retiré tant qu’il n’a pas été &quot;capturé&quot; par le marchand.<br/>**`Sale`** - Le montant de l’achat est autorisé et immédiatement retiré du compte du client. |
