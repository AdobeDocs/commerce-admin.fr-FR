...
| [!UICONTROL API Authentication Methods] | Site Web | Détermine la méthode utilisée pour l’authentification API. Options : <br/>**`API Signature`**- Affiche le champ _[!UICONTROL API Signature]_dans le formulaire.<br/>**`API Certificate`**- Affiche le champ_[!UICONTROL API Certificate]_ dans le formulaire. |
