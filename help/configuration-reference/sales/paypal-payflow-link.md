...
description: Vérifiez les paramètres de configuration dans la section [!UICONTROL PayPal Payflow Link] de la page [!UICONTROL Sales] &gt; [!UICONTROL Payment Methods] de l’administrateur Commerce.
...
| Utilisateur | Site Web | L’identifiant d’un utilisateur supplémentaire sur votre compte PayPal. S’il n’y a pas d’autres utilisateurs sur le réseau, saisissez votre identifiant de fournisseur ou de marchand. |
...
| [!UICONTROL Enable Payflow Link] | Site Web | Détermine si PayPal Payflow Link est disponible pour vos clients en tant que mode de paiement. |
| [!UICONTROL Enable Express Checkout] | Site Web | Détermine si le paiement express PayPal est disponible pour vos clients comme mode de paiement. |
...
| Champ | [Portée](../../getting-started/websites-stores-views.md#scope-settings) | Description |
|--- |--- |--- |
| [!UICONTROL Title] | Affichage en magasin | Nom qui identifie le lien de flux de paiement PayPal comme mode de paiement lors du passage en caisse. |
| [!UICONTROL Sort Order] | Affichage en magasin | Numéro qui détermine l’ordre dans lequel le lien de flux de paiement PayPal apparaît lorsqu’il est répertorié avec d’autres méthodes de paiement lors du passage en caisse. |
...
| [!UICONTROL URL Method for Cancel  URL and Return URL] | Site Web | Détermine la méthode utilisée pour exchange des informations avec le serveur PayPal lors d’une transaction. Options : <br/>**`GET`**- Récupère les informations qui résultent d’un processus. (Il s’agit de la méthode par défaut.)<br/>**`POST`** - Envoie un bloc de données, telles que les données entrées dans un formulaire, au processus de traitement des données. |
...
![Paramètres de base du paiement express PayPal](./assets/payment-methods-paypal-payflow-link-express-checkout-basic-settings.png)<!-- zoom -->
...
| Champ | [Portée](../../getting-started/websites-stores-views.md#scope-settings) | Description |
|--- |--- |--- |
| [!UICONTROL Display on Shopping Cart] | Affichage en magasin | Détermine si le paiement express PayPal apparaît comme option de paiement dans le panier. Options : Oui (recommandé) / Non |
