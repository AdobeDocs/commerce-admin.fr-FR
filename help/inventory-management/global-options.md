...
description: Configurez les paramètres  [!DNL Inventory Management]  catalogue global pour les options de stock de produits, les seuils et l’affichage du storefront sur tous les sites web.
...
Ces paramètres globaux s’appliquent à l’ensemble de votre catalogue sous **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**>**[!UICONTROL Catalog]**>**[!UICONTROL Inventory]**.
...
1. Développez ![Sélecteur d’extension](../assets/icon-display-expand.png) la section **[!UICONTROL Per-Source Availability (Storefront)]** et définissez **[!UICONTROL Enable sourceAvailability GraphQL Query]** sur `Yes` pour permettre à la requête storefront d’accéder aux données stock par source à l’aide de la requête [`sourceAvailability`](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/source-availability){target="_blank"} GraphQL. Ce paramètre est défini par vue de magasin.

   >[!NOTE]
   >
   >La requête `sourceAvailability` est désactivée par défaut, car elle indique quelles sources stockent un SKU. Les quantités exactes restent masquées jusqu&#39;à ce que la quantité source atteigne le **[!UICONTROL Only X left Threshold]** défini dans la section _[!UICONTROL Stock Options]_. Chaque origine de stock possède un indicateur de **[!UICONTROL Visible on Storefront]**désactivé par défaut. Le stock d’une source n’est jamais renvoyé tant qu’un commerçant ne l’a pas activée. Pour le définir, accédez à&#x200B;**[!UICONTROL Stores]**>_[!UICONTROL Inventory]_ > **[!UICONTROL Sources]**, modifiez une source, puis activez **[!UICONTROL Visible on Storefront]**.
