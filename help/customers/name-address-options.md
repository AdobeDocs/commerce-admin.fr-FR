...
La variable _Options de nom et d’adresse_ déterminer les champs inclus dans les formulaires de nom et d’adresse lorsque les clients créent une [account](../customers/account-create.md) avec votre magasin.
...
1. Développez l’objet **[!UICONTROL Name and Address Options]** .

   >[!INFO]
   >
   >Notez que la portée des options de nom et d’adresse s’applique à la variable `website` niveau.
...
1. Faites défiler la page jusqu’en haut de la page et définissez la portée de la configuration sur l’une des options suivantes :
   ...
   - `Default Config`
   - `Main Website` (ou site spécifique pour les installations multisite)

   >[!INFO]
   >
   >La variable _[!UICONTROL Name and Address Options]_n’apparaît pas lorsque la portée est définie sur `Default Store View`.

   ![Étendue de la configuration](assets/customer-configuration-scope-ee.png){width="700" zoomable="yes"}

### Étape 2 : Configuration des options de nom et d’adresse

1. Revenez au [!UICONTROL _Options de nom et d’adresse_] de la page Configuration client .

   >[!INFO]
   >
   > Si vous n’utilisez pas la variable `Default config` , vous devez effacer la variable `Use Default` pour chaque champ avant de modifier la valeur.
...
1. Dans le message situé en haut de la page, cliquez sur **[!UICONTROL Cache Management]** et [actualiser](../systems/cache-management.md) chaque cache non valide.
...
1. Faites défiler la page jusqu’en haut de la page et définissez la portée de la configuration sur l’une des options suivantes :
   ...
   >[!NOTE]
   >
   >La variable _Options de nom et d’adresse_ n’apparaît pas lorsque la portée est définie sur `Default Store View`.

   ![Étendue de la configuration](assets/configuration-scope.png){width="600" zoomable="yes"}
...
1. Pour **Nombre de lignes d’une adresse de rue**, saisissez un nombre compris entre 1 et 4.

   >[!WARNING]
   >
   >Par défaut, l&#39;adresse postale est de trois lignes.

1. Pour inclure un préfixe (tel que M. ou Mme) dans le nom, définissez **Afficher le préfixe** to `Yes`.

   ![Préfixe dans le formulaire d’inscription client](assets/storefront-customer-account-prefix.png){width="600" zoomable="yes"}

   >[!INFO]
   >
   >Pour **Options de liste déroulante des préfixes**, saisissez chaque préfixe à afficher dans la liste, séparé par un point-virgule. Vous pouvez placer un point-virgule avant la première valeur pour afficher une valeur vide en haut de la liste.

1. Pour inclure un champ facultatif pour le deuxième prénom ou l’initial du client, définissez **[!UICONTROL Show Middle Name (initial)]** to `Yes`.

1. Pour inclure un suffixe (tel que Jr. ou Sr) après le nom du client, définissez **[!UICONTROL Show Suffix]** à l’une des options suivantes :
   ...
   >[!INFO]
   >
   >Pour **Options du menu déroulant des suffixes**, saisissez chaque suffixe à afficher dans la liste, séparé par un point-virgule. Vous pouvez placer un point-virgule avant la première valeur pour afficher une valeur vide en haut de la liste.

1. Pour inclure la date de naissance, définissez **[!UICONTROL Show Date of Birth]** à l’une des options suivantes :
...
1. Pour permettre aux clients de saisir leur taxe ou [TVA](../stores-purchase/vat.md) number, set **[!UICONTROL Show Tax/VAT Number]** à l’une des options suivantes :

   - `Optional`
   - `Required`

1. Pour inclure un champ pour le genre dans le formulaire du client, définissez **[!UICONTROL Show Gender]** à l’une des options suivantes :
