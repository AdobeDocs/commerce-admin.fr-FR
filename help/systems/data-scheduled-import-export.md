...
1. Sur le _Administration_ barre latérale, accédez à **[!UICONTROL System]** > _[!UICONTROL Data Transfer]_>**[!UICONTROL Scheduled Imports/Exports]**.

   ![Import/export de données planifiées](./assets/data-scheduled-import-export.png){width="700" zoomable="yes"}
...
1. Lorsque l’enregistrement est enregistré, la tâche s’affiche dans le _[!UICONTROL Scheduled Import/Export]_grid.
...
1. Après chaque tâche planifiée, une copie du fichier est placée dans le `var/log/import_export` sur le serveur local Adobe Commerce.
...
Les détails de chaque opération d’importation ne sont pas écrits dans un journal, mais en cas d’échec, vous recevez une _Échec de l’importation_ avec une description de l’erreur. Le résultat de la dernière tâche d’importation planifiée s’affiche dans la colonne Dernier résultat de la page Importation/Exportation planifiée .

Après chaque opération d’importation, une copie du fichier d’importation est placée dans la variable `var/log/import_export` sur le serveur sur lequel Adobe Commerce ou Magento Open Source est déployé. L&#39;horodatage, le marqueur de l&#39;entité importée (produits ou clients) et le type d&#39;opération (dans ce cas, import) sont ajoutés au nom du fichier d&#39;import.
...
1. Sur le _Administration_ barre latérale, accédez à **[!UICONTROL System]** > _[!UICONTROL Data Transfer]_>**[!UICONTROL Scheduled Import/Export]**.
...
1. Définissez les options de planification et d’importation :
   ...
   - **[!UICONTROL Name]** — Saisissez le nom de l&#39;import planifié.

   - **[!UICONTROL Description]** — Saisissez une brève description qui explique l&#39;objectif de l&#39;import et comment il doit être utilisé.
   ...
   - **[!UICONTROL Import Behavior]** — Définissez sur l’une des options suivantes :
     ...
     >[!NOTE]
     >
     >Pour le _[!UICONTROL Advanced Pricing]_,_[!UICONTROL Products]_, _[!UICONTROL Customers and Addresses (single file)]_, et_[!UICONTROL Stock Sources]_ types d’entité, les comportements d’importation suivants s’affichent : `Add/Update`, `Replace`, et `Delete`. Pour le _Finances des clients_, _Fichier principal des clients_, et _Clients et adresses_ types d’entité, les comportements d’importation suivants s’affichent : `Add/Update Complex Data`, `Delete Entities`, et `Custom Action`.

   - **[!UICONTROL Start Time]** — Défini sur l’heure, la minute et la seconde auxquelles l’importation doit commencer.

   - **[!UICONTROL Frequency]** — Définissez sur l’une des options suivantes : `Daily`, `Weekly`, ou `Monthly`

   - **[!UICONTROL On Error]** - Définissez sur l’une des options suivantes : `Stop Import` ou `Continue Processing`

   - **[!UICONTROL Status]** — Pour activer l’importation planifiée, définissez sur `Enabled`.

   - **[!UICONTROL Field Separator]** — Saisissez le caractère utilisé pour séparer les champs dans le fichier d&#39;import. Le caractère par défaut est une virgule.
...
1. Définir **[!UICONTROL Server Type]** à l’une des options suivantes :
   ...
   ![Import de données - Informations sur les fichiers d’importation planifiés](./assets/data-transfer-scheduled-import-file-information.png){width="600" zoomable="yes"}

   >[!NOTE]
   >
   >Lorsque le module de stockage à distance est activé, `Local Server` bascule automatiquement vers `Remote Storage`.
...
1. Saisissez le **[!UICONTROL File Directory]** d’où provient le fichier d’importation.
   ...
   - `Remote FTP server` - Saisissez l&#39;URL complète et le chemin d&#39;accès au dossier d&#39;import sur le serveur distant.
...
1. Définir **[!UICONTROL Failed Email Receiver]** au contact du magasin qui doit recevoir la notification si une erreur se produit lors de l&#39;import.

1. Définir **[!UICONTROL Failed Email Sender]** au contact du magasin qui apparaît comme expéditeur de la notification.

1. Définir **[!UICONTROL Failed Email Template]** au modèle utilisé pour la notification.
...
1. Définir **[!UICONTROL Failed Email Copy Method]** à l’une des options suivantes :
...
1. Lorsque vous avez terminé, cliquez sur **[!UICONTROL Save]**.
   ...
   La nouvelle tâche d’importation planifiée est ajoutée à la liste du _[!UICONTROL Scheduled Import/Export]_page. À partir de cette page, il peut être exécuté immédiatement pour le test et modifié. Le fichier d&#39;import est validé avant l&#39;exécution de chaque traitement d&#39;import.
...
| [!UICONTROL Status] | L’importation est activée par défaut. Vous pouvez la suspendre en définissant l’état sur `Disabled`. |
...
| Champ | Description |
| ----- | ----------- | 
| [!UICONTROL Server Type] | Vous pouvez importer à partir d’un fichier sur le même serveur que celui sur lequel Commerce est déployé (sélectionnez `Local Server`) ou à partir du serveur FTP distant (sélectionnez `Remote FTP`). Si vous sélectionnez _[!UICONTROL Remote FTP]_, des options supplémentaires pour les informations d’identification et les paramètres de transfert de fichiers s’affichent. Si le module de stockage à distance est activé, `Local Server` est automatiquement activé sur `Remote Storage`. |
| [!UICONTROL File Directory] | Spécifiez le répertoire dans lequel se trouve le fichier d’importation. Si le type de serveur est défini sur _[!UICONTROL Local Server]_, indiquez le chemin d’accès relatif au répertoire d’installation de Commerce. Par exemple : `var/import` ou `import_export/import` pour le stockage à distance. |
...
L’exportation planifiée est similaire à un [Exporter](data-export.md) dans le format de fichier d&#39;export disponible et les types d&#39;entités pouvant être exportées :
...
Après chaque exportation, le fichier d’exportation est placé à l’emplacement défini par l’utilisateur, puis une copie dans la variable `var/log/import_export` sur le serveur sur lequel Adobe Commerce ou Magento Open Source est déployé. L’horodatage et le marqueur de l’entité exportée (produits ou clients) et le type d’opération (dans ce cas, export) sont ajoutés au nom du fichier d’export.
...
1. Dans le coin supérieur droit, cliquez sur **[!UICONTROL Add Scheduled Export]** et procédez comme suit :
   ...
   - Entrez un résumé **[!UICONTROL Description]** qui explique l’objectif de l’exportation et comment l’utiliser.

   - Définir **[!UICONTROL Entity Type]** à l’une des options suivantes :
     ...
     La variable _[!UICONTROL Entity Attributes]_La section située au bas de la page est mise à jour pour prendre en compte le type d’entité sélectionné.

   - Définir **[!UICONTROL Start Time]** à l’heure, à la minute et à la seconde auxquelles l’exportation est planifiée pour commencer.
...
1. Pour activer l’exportation planifiée, définissez **[!UICONTROL Status]** to `Enabled`.

1. Accepter `CSV` par défaut **[!UICONTROL File Format]**.
...
1. Définir **[!UICONTROL Server Type]** à l’une des options suivantes :
   ...
   >[!NOTE]
   >
   >Lorsque le module de stockage à distance est activé, la variable `Local Server` bascule automatiquement vers `Remote Storage`.

1. Pour **[!UICONTROL File Directory]**, saisissez le répertoire dans lequel le fichier d&#39;export doit être enregistré comme suit :

   - Pour **[!UICONTROL Local Server]**, saisissez un chemin relatif dans l’installation de Commerce, tel que `var/export`. Si le module de stockage distant est configuré, utilisez `import_export/export`.
   - Pour **[!UICONTROL Remote FTP server]**, saisissez l’URL complète et le chemin du dossier cible sur le serveur de destination.

1. Si la variable _[!UICONTROL Remote FTP]_est sélectionné, saisissez les informations de connexion au serveur et sélectionnez des paramètres supplémentaires :

   - Pour **[!UICONTROL FTP Host[:Port]]**, saisissez l’adresse d’hôte FTP distant.
   - Pour **[!UICONTROL User Name]**, saisissez le nom d’utilisateur utilisé pour accéder au serveur distant.
   - Pour **[!UICONTROL Password]**, saisissez le mot de passe du compte d’utilisateur fourni.
...
1. Définir **[!UICONTROL Failed Email Receiver]** au contact du magasin qui doit recevoir la notification si une erreur se produit lors de l’exportation.
...
1. Dans le _[!UICONTROL Entity Attributes]_, sélectionnez les attributs à inclure dans les données d’exportation.

   - Pour filtrer les données d’exportation par valeur d’attributs, saisissez la valeur d’attribut dans la variable _[!UICONTROL Filter]_colonne .
...
1. Lorsque vous avez terminé, cliquez sur **[!UICONTROL Save]**.
   ...
   La nouvelle tâche d’exportation planifiée est ajoutée à la liste du _[!UICONTROL Scheduled Import/Export]_page. À partir de cette page, il peut être exécuté immédiatement, testé et modifié.
...
| [!UICONTROL File Format] | Sélectionnez le format du fichier d&#39;export. Actuellement, seule la variable `.CSV` est disponible. |
...
| [!UICONTROL Failed Email Template] | Sélectionnez un modèle pour l’email d’exportation en échec. Par défaut, seule la variable `Export Failed (Default Template from Locale)` est disponible. |
