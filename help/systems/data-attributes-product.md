...
Les exemples de données sont installés dans l’installation utilisée pour exporter ces données. Deux sites web et plusieurs vues de magasin sont disponibles. Bien que cette liste inclut toutes les colonnes qui sont généralement exportées, `sku` est la seule valeur requise. Pour importer des données, vous pouvez inclure uniquement les colonnes avec modifications. `sku` doit être la première colonne, mais l’ordre du reste des attributs n’a pas d’importance.
...
| `product_type` | Indique le type de produit. Valeurs :<br/>`simple` — Éléments tangibles qui sont généralement vendus en unités uniques ou en quantités fixes.<br/>`grouped` — Groupe de produits distincts vendus en tant qu’ensemble.<br/>`configurable` — Produit avec plusieurs options que le client doit sélectionner avant d’effectuer un achat. Le stock peut être géré pour chaque ensemble de variations, car il représente un produit distinct avec un SKU distinct. Par exemple, une combinaison de couleur et de taille pour un produit configurable est associée à un SKU spécifique dans le catalogue.<br/>`virtual` — Produit non tangible qui ne nécessite pas d’expédition et qui n’est pas stocké. Par exemple, les services, les adhésions et les abonnements.<br/>`bundle` — Un ensemble de produits personnalisable de produits simples vendus ensemble. |
...
| `description` | La description du produit fournit des informations détaillées sur le produit et peut inclure des balises d’HTML simples. |
...
| save_rewrites_history | Lorsqu&#39;elle est fournie avec la valeur `1` avec un nouveau `url_key`, une nouvelle réécriture d&#39;URL 301 est générée afin que l&#39;ancienne URL soit redirigée vers la nouvelle URL. |
...
| `map_enabled` | Détermine si le prix publicitaire minimum est activé dans la configuration. Valeurs :<br/>`1` — (Oui) MAP est activé.<br/>`0` (ou vide) — (Non) MAP n’est pas activée. |
...
| `notify_on_stock_below` | Indique le niveau de stock qui déclenche une notification _out of stock_. |
...
| `crosssell_position` | Détermine la position (ordre de tri) des SKU répertoriés en tant que produits de vente croisée dans la colonne `crosssell_skus`. |
| `upsell_skus` | Répertorie le SKU de chaque produit qui a été identifié comme une vente ascendante. |
| `upsell_position` | Détermine la position (ordre de tri) des SKU répertoriés en tant que produits de vente incitative dans la colonne `upsell_skus`. |
...
| `_associated_sku` | SKU d’un produit associé à un produit groupé. | Produits groupés <br/>Produits groupés |
| `_associated_default_qty` | Détermine la quantité du produit associé qui est incluse. | Produits configurables<br/>Produits groupés<br/>Produits groupés |
...
L’importation/exportation de prix avancés vous permet de mettre rapidement à jour les informations de tarification pour les groupes de produits et les prix de niveau. Le processus de [import](data-import.md) et de [export](data-export.md) de données de prix avancées est le même que tout autre type d’entité. L’exemple de fichier CSV contient les prix des niveaux et des groupes pour chaque type de produit prenant en charge la tarification avancée. La modification des tarifs avancés n’a aucune incidence sur le reste de l’enregistrement du produit.

![Exemple de données d&#39;export - prix avancé](./assets/data-advanced-pricing-export-sample.png){width="600" zoomable="yes"}

| Attribut | Description |
|--- |--- |
| `sku` | (Obligatoire) L’unité de gestion des stocks est un identifiant alphanumérique unique utilisé pour effectuer le suivi du stock. Un SKU peut comporter jusqu’à 64 caractères. Par exemple : `sku123`<br/>**_Remarque :_**Un SKU de plus de 64 caractères provoque l’échec de l’importation. |
| `tier_price_website` | Le [code du site Web](../stores-purchase/stores.md#add-websites) identifie chaque site Web sur lequel des tarifs de niveau sont disponibles. Par exemple : `-  website1 -  All Websites [USD]` |
| `tier_price_customer` | Identifie les [groupes de clients](../customers/customer-groups.md) où le niveau de prix est disponible. Par exemple : `-  ALL GROUPS -  NOT LOGGED IN -  General -  Wholesale -  Retailer` |
...
| `tier_price` | Prix de niveau réduit du produit. Pour les [produits groupés](../catalog/product-create-bundle.md), le prix du niveau est calculé en pourcentage. |
| `group_price_website` | Le [code du site web](../stores-purchase/stores.md#add-websites) de chaque site web sur lequel la tarification de groupe est disponible. Si vous spécifiez plusieurs sites web, séparez-les par une virgule et sans espace. Par exemple : `-  website1 -  All Websites [USD]` |
...
| `group_price` | Prix de groupe réduit du produit. Pour les [produits groupés](../catalog/product-create-bundle.md), le prix du groupe est calculé en pourcentage. |
