...
description: En savoir plus sur les outils disponibles dans le [!DNL Page Builder] espace de travail lorsque vous créez des pages de base, des pages de produits et de catalogues, des blocs et des blocs dynamiques.
...
When [[!DNL Page Builder] est activé](setup.md), la variable _[!UICONTROL Content]_Le processus de création de section et de contenu est modifié pour tirer parti des [!DNL Page Builder] outils pour CMS [pages](../content-design/page-add.md), [product](../catalog/product-content.md) et [category](../catalog/categories-content-settings.md) pages, [blocs](../content-design/block-add.md), et [blocs dynamiques](../content-design/dynamic-blocks.md). Cette section comprend une_ En-tête du contenu _, un aperçu du contenu et un accès facile au mode plein écran [!DNL Page Builder] workspace.

![section Contenu avec [!DNL Page Builder] aperçu](./assets/pb-content-preview.png){width="700" zoomable="yes"}
...
>[!NOTE]
>
>La variable _[!UICONTROL Content Heading]_Le champ qui apparaît en haut de la page est un champ hérité qui prend en charge le contenu créé avec la version précédente. [!DNL Commerce] versions. Cela ne fait pas partie de [!DNL Page Builder]. La variable [!UICONTROL Content Heading] est formaté en tant qu’en-tête H1 en fonction de la feuille de style associée au thème actif. Il est positionné juste au-dessus de la zone de contenu active définie par la variable [!DNL Page Builder] scène.

Pour un meilleur contrôle sur le positionnement et le format des en-têtes de tous les niveaux, il est recommandé de laisser la variable _[!UICONTROL Content Heading]_vide et utilisez la variable [!DNL Page Builder] [En-tête](heading.md) type de contenu.

![En-tête de contenu et autres en-têtes](./assets/pb-storefront-heading-levels.png){width="700" zoomable="yes"}

## Aperçu

Lorsque vous développez l’objet _[!UICONTROL Content]_et qu’il existe du contenu créé avec [!DNL Page Builder], il affiche un aperçu du contenu tel qu’il apparaîtrait dans une page. Cliquez sur **[!UICONTROL Edit with Page Builder]**ou dans la zone d’aperçu du contenu pour ouvrir la [!DNL Page Builder] espace de travail, où vous pouvez effectuer les mises à jour nécessaires.
...
![Évaluation du contenu de la page](./assets/pb-workspace-simple-page.png){width="600" zoomable="yes"}

{{$include /help/_includes/page-builder-save-timeout.md}}

## Fenêtres d’affichage

A _viewport_ est la zone visible d’une page web qu’un utilisateur voit. En mode de conception en plein écran, les boutons de fenêtre d’affichage s’affichent au-dessus de la balise [!DNL Page Builder] étape pour vous montrer le contenu tel que l’utilisateur le voit sur le storefront.

![Boutons d’affichage](./assets/pb-workspace-viewport-buttons.png){width="500" zoomable="yes"}

[!DNL Page Builder] définit également des points d’arrêt pour les fenêtres d’affichage. Les points d’arrêt définissent les largeurs minimales et maximales au sein desquelles certains styles sont appliqués. La variable [!DNL Page Builder] Les fenêtres d’affichage fournissent les points d’arrêt de contenu suivants :
...
[!DNL Page Builder] les fenêtres d’affichage proposent deux fonctionnalités : **_aperçus de contenu_** et **_paramètres des points d’arrêt_**.
...
Les boutons de fenêtre d’affichage offrent également la possibilité d’appliquer différents styles de point d’arrêt aux types de contenu en fonction de la fenêtre d’affichage sélectionnée. Par défaut, [!DNL Page Builder] fournit les paramètres de point d’arrêt pour la variable _[!UICONTROL Minimum Height]_champs de lignes, colonnes, onglets, éléments d’onglet, bannières, curseurs et diapositives. Lorsque vous sélectionnez la fenêtre d’affichage Mobile, puis ouvrez l’éditeur pour l’un de ces types de contenu, vous pouvez saisir des valeurs de champ spécifiques aux points d’arrêt de la fenêtre d’affichage Mobile . Les champs de type Contenu qui autorisent des paramètres de point d’arrêt spécifiques affichent une icône à droite du champ, comme dans l’exemple suivant pour une ligne :

![Indicateur d’icône pour le paramètre du point d’arrêt](./assets/pb-workspace-viewport-field-breakpoint.png){width="400"}

## Panneau

La variable [!DNL Page Builder] situé à gauche de l’étape. Il contient des types de contenu qui peuvent être déplacés vers l’étape. Un conteneur spécifique au type de contenu s’affiche alors avec une boîte à outils d’options. Les types de contenu sont organisés dans le panneau comme suit :

### Disposition

La variable _[!UICONTROL Layout]_de la [!DNL Page Builder] permet d’ajouter des lignes, des colonnes ou des onglets à la scène. Lorsque vous faites glisser un type de contenu du panneau vers la scène, un conteneur s’affiche avec une boîte à outils d’options spécifiques au type de contenu.

Par défaut, la variable [!DNL Page Builder] stage est vide. Lorsque vous faites glisser des types de contenu de mise en page du panneau vers la scène, vous pouvez les placer au-dessus, en dessous ou à l’intérieur d’autres conteneurs de mise en page sur la page. Les lignes ne peuvent être ajoutées directement à la scène que.

![[!DNL Page Builder] panneau avec les types de contenu de mise en page et étape](./assets/pb-stage-toolbox.png){width="600" zoomable="yes"}
...
| [Colonne](column.md) | Vous pouvez faire glisser une colonne du panneau vers la scène ou vers des lignes et des onglets. Le nombre maximal de colonnes pouvant être ajoutées est déterminé par le nombre de divisions de grille spécifiées dans la variable [configuration](setup.md). |
...
Utilisez la variable _[!UICONTROL Elements]_de la [!DNL Page Builder] pour ajouter du texte, des en-têtes, des boutons, des séparateurs et du code de HTML à n’importe quel conteneur de mises en page du panneau [[!DNL Page Builder] étape](workspace.md#stage). Lorsque vous faites glisser un type de contenu du panneau vers une ligne ou une colonne, ou vers un ensemble d’onglets sur la scène, un conteneur s’affiche. Utilisez la boîte à outils de type de contenu pour accéder aux paramètres spécifiques au type.
...
| [En-tête](heading.md) | Ajoute un conteneur d’en-tête à la scène. |
...
| [Code HTML](html-code.md) | Ajoute un conteneur pour le code de HTML à la scène. |

{style="table-layout:auto"}

### Média

Utilisez la variable _[!UICONTROL Media]_de la [!DNL Page Builder] pour ajouter des images, des vidéos, des bannières, des curseur et des [!DNL Google Maps] à n’importe quel conteneur de mises en page de [[!DNL Page Builder] étape](workspace.md#stage). Lorsqu’un type de contenu multimédia est glissé du panneau vers la scène, un conteneur s’affiche avec une boîte à outils contenant les options propres au type de contenu.
...
| [Carte](map.md) | Ajoute un [!DNL Google Maps] conteneur à l’étape. |

{style="table-layout:auto"}

### Ajouter du contenu

Utilisez la variable _[!UICONTROL Add Content]_de la [!DNL Page Builder] pour ajouter du contenu existant au panneau [[!DNL Page Builder] étape](workspace.md#stage). Lorsque vous faites glisser un type de contenu multimédia du panneau vers la scène, un conteneur s’affiche. Utilisez la boîte à outils de type de contenu pour accéder au_ Paramètres _qui sont spécifiques au type.

![[!DNL Page Builder] panneau avec types Ajouter du contenu](./assets/pb-add-content.png){width="600" zoomable="yes"}
...
![Options de la boîte à outils Rangées](./assets/pb-tutorial1-row-toolbox.png){width="600" zoomable="yes"}
