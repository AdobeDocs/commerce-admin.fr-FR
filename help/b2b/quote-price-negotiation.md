...
If [Les guillemets B2B sont activés](configure-quotes.md) dans la configuration, la négociation des prix peut être initiée par un acheteur autorisé d’une société ou d’un représentant commercial.

Les acheteurs lancent le processus de négociation des prix en [demande d’un devis](quote-request.md) du panier. Les représentants commerciaux peuvent lancer des négociations par [création d&#39;un projet de devis pour un acheteur](sales-rep-initiates-quote.md), en mettant à jour le devis avec les articles de commande et le prix initiaux, et en l’envoyant à l’acheteur.

Lorsque la négociation des prix démarre, les guillemets sont répertoriés dans la variable [Guillemets](quotes.md) grid. Toutes les négociations entre l&#39;acheteur et le vendeur se font par email, et sont initiées et suivies depuis le détail du devis.
...
Les acheteurs gèrent le processus de négociation des devis à partir du storefront en utilisant [[!UICONTROL My Quotes]](account-dashboard-my-quotes.md). Bien que le devis soit ouvert à révision, son statut dans le compte de l’acheteur est défini sur `Pending`. L&#39;acheteur peut modifier et soumettre à nouveau le devis même s&#39;il a été refusé ou s&#39;il a expiré.

## Étape 1 : afficher la requête

1. Dans la barre latérale d’administration, accédez à **[!UICONTROL Sales]** > **[!UICONTROL Quotes]**.

   La nouvelle requête apparaît dans la variable _[!UICONTROL Quotes]_grid.

1. Dans le _Actions_ colonne, cliquez sur **[!UICONTROL View]**.

   ![Nouvelle citation](./assets/quote-grid-new.png){width="700" zoomable="yes"}

## Etape 2 : modifier le guillemet

1. Sous _[!UICONTROL Quote & Account Information]_, cliquez sur le_ Calendrier _(![Icône Calendrier](../assets/icon-calendar.png)).

   ![Informations sur les devis et les comptes](./assets/quote-details-account-information.png){width="575" zoomable="yes"}

1. Choisissez une **[!UICONTROL Expiration Date]** pour la citation.

1. Faites défiler l’écran vers le bas jusqu’à _[!UICONTROL Quote Totals]_et mettre à jour la section **[!UICONTROL Negotiated Price]**selon les besoins.
...
1. Saisissez le **[!UICONTROL SKU]** et **[!UICONTROL Qty]** à ajouter.
...
Appliquez les modifications d’élément de ligne dans la variable _[!UICONTROL Items Quoted]_si nécessaire.

![Application des mises à jour des éléments de ligne](./assets/quote-apply-line-item-operations.png){width="600" zoomable="yes"}

- Modifiez la variable **[!UICONTROL Quantity]** qui doit être acheté au Prix proposé.

- Sélectionner **[!UICONTROL Configure]** et modifiez les options du produit.

  La variable [!UICONTROL Configure] option disponible uniquement sur un élément de ligne pour un produit configurable

- Dans le **[!UICONTROL Action]** , sélectionnez une action pour mettre à jour l’élément :
   - **Article de réduction** pour appliquer une remise sous forme de pourcentage, de montant fixe ou de prix préférentiels.
Vous pouvez éventuellement verrouiller le montant de la remise pour éviter toute remise supplémentaire. Si la remise n’est pas verrouillée, la remise sur une ligne et la remise sur un prix sont appliquées au prix du produit.
   - **Laisser une note à l’acheteur** fournir à l’acheteur des informations supplémentaires sur un article ;
...
- Pour appliquer les modifications et mettre à jour le devis au catalogue partagé et aux règles de prix, cliquez sur **[!UICONTROL Update Prices]** puis cliquez sur **[!UICONTROL Proceed]** pour confirmer la mise à jour.

  ![Éléments cités](./assets/quote-detail-items-quoted.png){width="600" zoomable="yes"}

### Mise à jour des informations d’expédition

1. Si l’acheteur inclut un _Ship To_ dans la citation, cliquez sur **[!UICONTROL Get shipping methods and rates]**.

1. Choisissez un mode de livraison parmi les options disponibles.

1. Saisissez un **[!UICONTROL Proposed Shipping Price]**.

   La variable _[!UICONTROL Quote Totals]_sont mises à jour pour prendre en compte le prix d’expédition proposé.

### Joindre un document de prise en charge

1. Sous , _Ajouter votre commentaire_ , cliquez sur **[!UICONTROL Attach file]**.

   Par défaut, [fichiers joints](../configuration-reference/sales/quotes.md) peut contenir jusqu’à 2 Mo dans les formats de fichiers suivants : DOC, DOCX, XLS, XLSX, PDF, TXT, JPG ou JPEG, PNG.
...
1. Dans le _[!UICONTROL Negotiation]_de la section_[!UICONTROL Comments]_ , saisissez votre réponse dans l’onglet **[!UICONTROL Add your comment]** .
...
1. Pour appliquer une remise au devis dans son ensemble :
   ...
   - Sous _[!UICONTROL Quote Totals]_dans le_[!UICONTROL Negotiated Price]_ , sélectionnez l’un des types de remise suivants :
   ...
   - Saisissez le montant en pourcentage ou en prix forfaitaire.
     ...
     ![Commentaires sur les négociations](./assets/quote-detail-negotiation-comments.png){width="600" zoomable="yes"}

1. Envoyez ou enregistrez le guillemet :

   - Si le devis est prêt à être renvoyé à l’acheteur, cliquez sur **[!UICONTROL Send]**.

   - Pour continuer à travailler ultérieurement sur le guillemet, cliquez sur **[!UICONTROL Save as Draft]**.
...
| Bouton | Description |
|----------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Back] | Renvoie à la variable _[!UICONTROL Quotes]_sans enregistrer les modifications. |
| [!UICONTROL Print] | Envoie le guillemet à une imprimante ou l’enregistre sous la forme d’un fichier de PDF. |
| [!UICONTROL Create Copy] | [!BADGE Fonctionnalités 1.5.0 bêta]{type=Informative url="/help/b2b/release-notes.md" tooltip="Disponible uniquement pour les participants au programme bêta"}`(copy)` ajouté au nom d’origine. Renommez le nouveau guillemet en modifiant le [!UICONTROL Name] champ . Traitez le nouveau guillemet en l’enregistrant en tant que brouillon ou en l’envoyant au client. |
...
| Champ | Description |
|-------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Name] | Nom affecté à une requête de devis par [acheteur](account-company-roles-permissions.md). |
| [!UICONTROL Status] | Indique l’état actuel du guillemet. Le statut d&#39;un devis ne peut être modifié que par une action de la part de l&#39;acheteur ou du vendeur. Voir aussi [Paramètres d’état](quotes.md) de l’Admin et de la [compte de l&#39;acheteur](account-dashboard-my-quotes.md). |
...
| [!UICONTROL Expiration Date] | Indique le dernier jour de validité du guillemet actuel. La date d’expiration par défaut est définie dans la configuration sur 30 jours après l’envoi par un acheteur d’une demande de devis. <br/><br/>Le vendeur peut remplacer la date d’expiration par défaut en saisissant une autre date (MMM JJ AAAA ) ou en choisissant une dans le calendrier. Le guillemet n’expire jamais si le champ est laissé vide. <br/><br/>Pour les devis ouverts, le vendeur reçoit une [notification par email](../systems/email-templates.md) 48 heures avant l’expiration du guillemet. Les acheteurs sont avertis 24 heures avant la date d&#39;expiration. <br/><br/>Le statut du guillemet change en _Expiré_ et l&#39;acheteur ne peut pas apporter d&#39;autres modifications au devis. Les prix proposés dans le devis reviennent aux valeurs originales du catalogue. <br/><br/>Si un guillemet est ouvert pour révision par le vendeur lorsque le guillemet est défini pour expirer, la date d’expiration est réinitialisée selon la plage définie dans la configuration. <br/><br/>La Date d’expiration est le seul champ de la variable _Devis et compte_ qui peuvent être modifiées au cours du processus de révision. |
| [!UICONTROL Company] | Nom légal de la variable [société](account-companies.md) que l&#39;acheteur représente. |
| [!UICONTROL Company Admin Email] | L’adresse électronique du [administrateur de société](account-company-admin.md). |
| [!UICONTROL Sales Rep] | La variable [représentant commercial](account-company-manage.md) qui travaille pour le vendeur et est le contact principal affecté au compte de la société. |
| [!UICONTROL Shared Catalog (or Customer Group)] | La variable [catalogue partagé](catalog-shared.md) ou [groupe de clients](account-company-customer-group.md) à laquelle la société est affectée. Le devis peut inclure des prix personnalisés du catalogue partagé affecté à la société. |
...
| [!UICONTROL Shipping Method & Price] | Le lien Obtenir les tarifs et les méthodes d’expédition s’affiche si l’acheteur inclut une _Ship To_ dans la citation. |
...
| Champ | Description |
|--------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [!UICONTROL Comments] | L&#39;onglet Commentaires de la section Négociation permet de saisir un message à l&#39;acheteur au sujet du devis. <br/>**[!UICONTROL Add your comment]**- Les commentaires sont utilisés pour communiquer avec l&#39;acheteur pendant le processus de négociation. Utilisez les commentaires pour expliquer les remises proposées dans le devis, ou la raison pour laquelle une demande de devis est refusée.<br/>**[!UICONTROL Attach file]** - Taille de fichier maximale et types de fichiers pris en charge pour [fichiers joints](configure-quotes.md) sont déterminés par la configuration. Par défaut, un fichier joint peut contenir jusqu’à 2 Mo et l’un des types de fichiers suivants : DOC, DOCX, XLS, XLSX, PDF, TXT, JPG ou JPEG, PNG. |
...
| [!UICONTROL Estimated Tax] | Montant de la taxe estimée à payer, comme indiqué dans le paramétrage [paramètres d&#39;affichage](../configuration-reference/sales/tax.md). |
