...
Adobe Commerce et Magento Open Source peuvent être configurés pour diffuser toutes les pages `HTTPS` par défaut. Si votre boutique a fonctionné avec le protocole standard, vous pouvez améliorer la sécurité en activant [HTTP Strict Transport Security][2] (HSTS) et de mettre à niveau toutes les demandes de page non sécurisées. HSTS est un protocole d’opt-in qui empêche les navigateurs de rendre les normes `HTTP` pages transmises avec un protocole non sécurisé pour le domaine spécifié. Parce que les moteurs de recherche ont peut-être déjà indexé chaque page de votre magasin avec la méthode standard `HTTP` URL, vous pouvez configurer Commerce pour mettre à niveau toute demande de page non sécurisée vers `HTTPS` automatiquement , afin de ne pas perdre de trafic. Lorsque Commerce est configuré pour utiliser des URL sécurisées pour le storefront et l’administrateur, deux champs supplémentaires s’affichent pour vous permettre d’activer `HSTS`.
...
1. Développer ![Sélecteur d’extension](../assets/icon-display-expand.png) la valeur **[!UICONTROL Base URL]** .
   ...
   - **[!UICONTROL Base URL]** — Saisissez l’URL de base complète pour votre magasin. Veillez à terminer l’URL par une barre oblique afin qu’elle puisse être étendue à l’aide de clés URL supplémentaires provenant de votre magasin. Par exemple: `http://yourdomain.com/`
     ...
     >[!NOTE]
     >
     >Ne modifiez pas l’espace réservé dans la variable _[!UICONTROL Base Link URL]_champ . Il s’agit d’un espace réservé utilisé pour créer des liens relatifs vers l’URL de base.

   - **[!UICONTROL Base URL for Static View Files]** — (Facultatif) Spécifiez un autre emplacement pour l’URL de base pour les fichiers d’affichage statique en saisissant le chemin commençant par l’espace réservé suivant :
...
1. Développer ![Sélecteur d’extension](../assets/icon-display-expand.png) la valeur _[!UICONTROL Base URLs (Secure])_ et procédez comme suit :
   ...
   - **[!UICONTROL Secure Base URL for Static View Files]** — (Facultatif) Spécifiez un autre emplacement pour l’URL de base sécurisée pour les fichiers d’affichage statique en saisissant le chemin commençant par l’espace réservé suivant :

     \{\{secure_base_url}}

   - **[!UICONTROL Secure Base URL for User Media Files]** — (Facultatif) Indiquez un autre emplacement pour l’URL de base sécurisée pour les fichiers multimédias utilisateur en saisissant le chemin commençant par l’espace réservé suivant :
...
1. Pour _[!UICONTROL Enhanced Security Settings]_, procédez comme suit :
   ...
   - **[!UICONTROL Enable HTTP Strict Transport Security (HSTS)]** — Si vous souhaitez que votre boutique affiche uniquement les demandes de page HTTPS sécurisées, définissez sur `Yes`.

   - **[!UICONTROL Upgrade Insecure Requests]** — Pour mettre à niveau toute demande de pages HTTP non sécurisées standard pour sécuriser HTTPS, définissez sur `Yes`.

1. Définissez la variable **[!UICONTROL Offloader Header]** pour votre serveur.
...
>[!NOTE]
>
>Lorsque la variable _Ajout de code de magasin aux URL_ est définie sur `Yes`, vous devez inclure les codes de magasin dans les URL de votre navigateur. Ce paramètre garantit que les réécritures d’URL sont correctement mappées et que toutes les pages sont ouvertes correctement, sans _&quot;404 Page introuvable&quot;_ erreurs.

1. Sur le _Administration_ barre latérale, accédez à **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**.

1. Sous _[!UICONTROL General]_dans le panneau de gauche, choisissez **[!UICONTROL Web]**.

1. Développer ![Sélecteur d’extension](../assets/icon-display-expand.png) la valeur **[!UICONTROL URL Options]** .

1. Définir **[!UICONTROL Add Store Code]** selon vos préférences :
...
1. Cliquez sur le bouton **[!UICONTROL Cache Management]** dans le message en haut de l’espace de travail. Suivez ensuite les instructions pour actualiser le cache.
...
- Sur le serveur, modifiez la variable `.htaccess` (ou équilibreur de charge) afin que l’URL non sécurisée soit redirigée vers l’URL sécurisée.
...
- **Chemin d’accès d’administration par défaut**: `admin`
- **URL et chemin d’accès administrateur par défaut**: `http://yourdomain.com/magento/admin`
...
1. Développer ![Sélecteur d’extension](../assets/icon-display-expand.png) la valeur **[!UICONTROL Admin Base URL]** .

1. Définissez les options de configuration de l’URL personnalisée :

   ![Configuration avancée - URL de la base d’administration](../configuration-reference/advanced/assets/admin-admin-base-url.png){width="600" zoomable="yes"}

   Si nécessaire, effacez la variable **[!UICONTROL Use system value]** pour modifier le paramètre.

   - Définir **[!UICONTROL Use Custom Admin URL]** to `Yes`.
   ...
   - Définir **[!UICONTROL Custom Admin Path]** to `Yes`.

   - Pour **[!UICONTROL Custom Admin Path]**, saisissez le chemin à utiliser comme nom de dossier d’administration personnalisé.
...
1. Ouvrez le `app/etc/env.php` dans un éditeur de texte, puis modifiez la valeur de la variable `frontName` du paramètre `backend` . Enregistrez ensuite le fichier.
   ...
   >[!TIP]
   >
   >Pour Adobe Commerce sur l’infrastructure cloud, vous pouvez configurer un chemin d’accès d’administrateur personnalisé à l’aide de la variable `ADMIN_URL` dans l’interface utilisateur de Cloud. Voir [Rubrique Variables d’administration](https://experienceleague.adobe.com/docs/commerce-cloud-service/user-guide/configure/env/stage/variables-admin.html) dans le _Guide d’infrastructure de Commerce on Cloud_.

   - **Chemin d’accès d’administration par défaut**
   ...
   - **Nouveau chemin d’accès administrateur**
...
1. Utilisez l’une des méthodes suivantes pour effacer le cache :
   ...
   - Sur le _Administration_ barre latérale, accédez à **[!UICONTROL System]** > _[!UICONTROL Tools]_>**[!UICONTROL Cache Management]**. Cliquez ensuite sur **[!UICONTROL Flush Magento Cache]**.
   ...
   >[!NOTE]
   >
   >Les modifications effectuées à l’aide de la méthode 1 ont la priorité sur les modifications apportées dans la variable `app/etc/env.php` fichier .

### Méthode 3 : modifiez le chemin d’accès administrateur à l’aide de l’interface de ligne de commande de Commerce

Vous pouvez utiliser l’interface de ligne de commande `setup:config:set` pour modifier le chemin d’accès de l’administrateur. L’exemple suivant utilise la méthode `--backend-frontname` pour modifier le chemin d’accès de la racine Commerce vers un nouveau chemin d’accès administrateur :

```terminal
bin/magento setup:config:set --backend-frontname="backend_front_name"
```

Cette commande met à jour la variable `backend` > `frontName` de la section `app/etc/env.php` fichier .
...
1. Pour rétablir le chemin d’accès d’administrateur par défaut (défini dans la variable `app/etc/env.php` comme décrit dans la Méthode 2), exécutez cette commande :
