...
description: Découvrez comment les clients peuvent utiliser des méthodes de paiement stockées sur votre vitrine Commerce.
...
Les clients qui ont accès à un coffre sécurisé pour stocker des informations de paiement peuvent accélérer leur passage en caisse sans saisir à chaque fois leurs informations de carte de crédit. If [achat instantané](checkout-instant-purchase.md) est activée, les clients peuvent ignorer le processus de paiement en deux étapes et passer la commande à partir de la page de produits.

Un mode de paiement prenant en charge un coffre sécurisé, tel que [Braintree](braintree.md), est obligatoire. Lorsqu’un coffre sécurisé est activé dans la configuration du mode de paiement, les clients ont la possibilité, lors de leur passage en caisse, d’enregistrer leurs informations de carte de crédit comme méthode de paiement stockée. Les clients peuvent gérer les modes de paiement stockés à partir du tableau de bord de leur compte.
...
1. Termine la variable _Expédition_ étape .

1. Sélectionne la variable **[!UICONTROL Braintree Credit Card]** mode de paiement.

1. Remplit les données de carte de crédit.

1. Sélectionne la variable **[!UICONTROL Save for later use]** .

1. Clics **[!UICONTROL Place Order]**.

Le mode de paiement enregistré s’affiche alors dans la variable _[!UICONTROL Stored Payment Methods]_de l’onglet du tableau de bord du client.
