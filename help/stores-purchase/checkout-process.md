...
Lorsque le processus de passage en caisse commence, la transaction passe à un canal sécurisé et chiffré. Un symbole de cadenas s’affiche dans la barre d’adresse du navigateur et l’URL change à partir de `http` to `https`.

## Processus

L’objectif du processus de passage en caisse est de collecter les informations nécessaires pour terminer la transaction. La variable _Passage en caisse_ guide le client à chaque étape du processus. Les clients qui sont connectés à leur compte peuvent effectuer rapidement l’extraction, car la plupart des informations se trouvent déjà dans leurs comptes. Les clients associés à un compte d’entreprise qui utilise des commandes d’achat ont un workflow légèrement différent.
...
![Page d’expédition pendant le processus de paiement](./assets/storefront-checkout-step1-shipping.png){width="600" zoomable="yes"}
...
1. Au bas de la _[!UICONTROL Shipping Address]_, le client clique sur **[!UICONTROL + New Address]**.

1. Termine la variable _[!UICONTROL Shipping Address]_formulaire.
...
1. Dans la liste de [shipping](delivery.md) , le client choisit l’option qu’il souhaite utiliser.
...
Au cours de la deuxième étape du processus de passage en caisse, le client choisit la variable [mode de paiement](payments.md)et applique à l’achat les coupons comportant des codes promotionnels. Toutes les informations peuvent être examinées et modifiées si nécessaire. S’il est activé, le client doit accepter les conditions générales de la vente avant de passer la commande.
...
1. Le client sélectionne un mode de paiement.
   ...
   Lors de l’utilisation de la variable _Paiement sur compte_ , [!UICONTROL Custom Reference Number] peut être utilisé pour référencer un numéro de facture.

1. Le client clique **[!UICONTROL Place Purchase Order]**.

   Le bon de commande est passé.

Si l’entreprise a été configurée [règles de validation](../b2b/account-dashboard-approval-rules.md), le bon de commande passe par le processus d’approbation. Sinon, il est traité immédiatement.

![Révision et paiement des commandes d’achat](./assets/checkout-storefront-step2-b2b.png){width="700" zoomable="yes"}
...
La page de confirmation de commande est également appelée _Succès_ et est utilisé par les programmes analytics pour effectuer le suivi des conversions.

![La page de confirmation de commande affiche un message de réussite et un numéro de commande.](./assets/storefront-checkout-confirmation-customer.png){width="700" zoomable="yes"}
...
1. Si les paramètres correspondent à une vue de magasin spécifique, [choix de la vue magasin](../configuration-reference/scope-change.md#set-the-scope) où la configuration s’applique.
...
|--- |--- |--- |
| [!UICONTROL Enable Onepage Checkout] | Affichage en magasin | Détermine si [passage en caisse d’une page](checkout-one-page.md) est le format de passage en caisse par défaut. Options : Oui/Non |
| [!UICONTROL Allow Guest Checkout] | Affichage en magasin | Détermine si les invités peuvent passer [passage en caisse sans enregistrement](checkout-guest.md) pour un compte avec votre boutique. Options : `Yes` / `No` |
| [!UICONTROL Enable Terms and Conditions] | Affichage en magasin | Détermine si les clients doivent accepter le [Termes et conditions](terms-and-conditions.md) de la vente avant d’effectuer un achat. Options : `Yes` / `No` |
