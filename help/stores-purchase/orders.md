...
La grille _Commandes_ répertorie toutes les commandes en cours et suit leur progression et [état de la commande](order-status.md) via le [workflow](order-processing.md). Pour comprendre facilement le processus de base, une commande devient une [facture](invoices.md), et une facture devient une [expédition](shipments.md). La grille représente la première étape du processus. C’est là que vous pouvez [mettre à jour](order-update.md) les commandes existantes et créer des commandes.
...
Dans le coin supérieur droit, cliquez sur le contrôle _Colonnes_ ( ![Paramètres de colonne](../assets/icon-columns.png) ) et procédez comme suit :
...
1. Cliquez sur la commande _Colonnes_ ( ![Paramètres de colonne](../assets/icon-columns.png) ).

1. Pour réinitialiser les colonnes de la grille, cliquez sur **[!UICONTROL Reset]**.

   La mise en page de la grille change pour afficher uniquement les [colonnes par défaut](#column-descriptions).
...
1. Cliquez sur la commande **[!UICONTROL View]** ( ![Icône OEil](../assets/icon-view-eye.png) ).
...
Cliquez sur la commande **[!UICONTROL View]** ( ![Icône OEil](../assets/icon-view-eye.png) ). Ensuite, effectuez l’une des opérations suivantes :
...
| [!UICONTROL Columns] | Détermine la sélection des colonnes et leur ordre dans la grille. La mise en page des colonnes peut être modifiée et enregistrée en tant que _vue_. Par défaut, seules certaines colonnes sont incluses dans la grille. |
...
1. Pour afficher la sélection des filtres de recherche, cliquez sur l’onglet _Filtres_ ( ![Icône Entonnoir](../assets/icon-filter-search.png) ).
...
- [De nouvelles commandes sont envoyées vers l’archive](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/known-issues-patches-attached/new-orders-are-sent-to-archive.html)
